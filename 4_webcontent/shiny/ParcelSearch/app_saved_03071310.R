                                                   ...
                                                   fluidRow(
                                                           ...
                                                           column(width = 3, offset = 1,
                                                                  div(style = "margin-top: 10px;",
                                                                      HTML(paste0('<button data-toggle="collapse" data-target="#dtPrev">',icon("table"),' Expand</button>'))))
                                                   ),
                                                   fluidRow(
                                                           column(width = 12,
...
# +-- Controls ------------------------------------
                                                        fluidRow(column(width = 12,
                                                                        ...
                                                                        fluidRow(
                                                                                ...
                                                                                column(width = 3, offset = 1,
                                                                                       div(style = "margin-top: 10px;",
                                                                                           HTML(paste0('<button data-toggle="collapse" data-target="#dtPreview">',icon("table"),' Expand</button>'))))
                                                                              ),
                                                                        fluidRow(
                                                                                column(width = 12,
                ...
                tabPanel(title = "Notes",
                         ...
                         fluidPage(
                                 ...
                                 column(width = 3,
                                        ...
                                        h6(br()),
                                        # h5("Data Summary"),
                                        HTML(paste0('<button data-toggle="collapse" data-target="#glimpse">',
                                                    icon("table"),
                                                    ' Data Summary</button>')),
                                        h6(br()),
...
# +-- Map1 ----
        ...
        rv1 <- reactiveValues(shp = parcel_ycc_reduc,
                              ...
                              uv = unique(parcel_ycc_reduc@data$UV_TR)
        )
      
        filter_shp1 <- function(){
                
                rv1$shp <- parcel_ycc_reduc %>% subset(subset = UV_TR %in% rv1$uv)
        ...
        observeEvent(input$clustOpts,{
                ...
                if(input$clustOpts == T)
                        return(leafletProxy(mapId = "map2") %>%
                             ...
                             clearMarkers() %>%
                             clearMarkerClusters() %>% 
                             addMarkers(data = rv$cnts,popup = popup))
        })
...
# +-- Data Table (Map2) -------
        ...
        output$dt <- DT::renderDataTable(server = FALSE, selection = 'single',{
                ...
                rv$df %>%
                        as.data.frame() %>%
                        select('Property Name' = PROP_NAME,'Max. Floor Area' = ADJRCAP_FL_AREA_MAX,'Appr. Land Value' = LAND_AV, 'Appr. Bldg. Value' = BLDG_AV) %>% 
                        DT::datatable(selection = 'single',options = list(pageLength = 5))
