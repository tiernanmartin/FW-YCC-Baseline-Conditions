...
#  +--- UI Switch (Map2) ---------
        ...
        output$units <- renderUI({
                if (input$input_type == "Numeric")
                        ...
                        div(strong("Maximum Potential Units Range"),
                            sliderInput("units", "",
...
#  +--- Map Content -----
        ...
        output$map <- renderLeaflet({
                ...
                myLfltShiny() %>%
                        setView(lng = bounds_ycc_cntr[1],lat = bounds_ycc_cntr[2],zoom = 10) %>% 
        ...
        observeEvent(input$remove,{
                leafletProxy(mapId = "map") %>% hideGroup("highlight") %>% clusters()
        })
#  +--- Data Table --------
        output$dt <- DT::renderDataTable(server = FALSE, selection = 'single',{
                ...
                rv$df %>%
                        as.data.frame() %>%
                        select('Property Name' = PROP_NAME,'Zoning' = ZONING,'Units' = POT_UNITS,'Max. Floor Area' = ADJRCAP_FL_AREA_MAX,'Appr. Land Value' = LAND_AV, 'Appr. Bldg. Value' = BLDG_AV) %>%
